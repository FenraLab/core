...
export type TFunction<TArguments extends any[], TError = any> = (
  ...args: [...TArguments, TNext<TError>]
) => TReturn;
export type TMethod<
  TBaseclass,
  TArguments extends any[],
  TError = any,
> = TFunction<[TBaseclass, ...TArguments], TError>;

export function emptynext<TError = any>(error?: TError) {}

export class Manager<TArguments extends any[], TError = any> {
...
export async function invoke<TArguments extends any[] = [], TError = any>(
  middlewares: TFunction<TArguments, TError>[],
  ...args: TArguments
): Promise<void> {
